...
class Anuncios {
	...
	public function getTotoalAnuncios($filtros) {
		global $pdo;
	...
	public function getUltimosAnuncios($page, $perPage, $filtros){
		...
		$sql = $pdo->prepare("SELECT 
			*,
			(select anuncios_imagens.url from anuncios_imagens where anuncios_imagens.id_anuncios = anuncios.id limit 1) as url,
	...
	public function getAnuncio($id) {
		...
		if ($sql->rowCount() > 0) {
			...
			$sql = $pdo->prepare("SELECT id, url FROM anuncios_imagens WHERE id_anuncios = :id_anuncios");
		...
		if (count($fotos) > 0) {
			for ($q = 0; $q < count($fotos['tmp_name']); $q++) {
				...
				if (in_array($tipo, array('image/jpeg', 'image/png'))) {
					...
					$sql = $pdo->prepare("INSERT INTO anuncios_imagens SET id_anuncios = :id_anuncios, url = :url");
					$sql->bindValue(":id_anuncios", $id);
	...
	public function excluirAnuncio($id) {
		...
		// Removendo as imgs do anúnio
		$sql = $pdo->prepare("DELETE FROM anuncios_imagens WHERE id_anuncios = :id_anuncio");
